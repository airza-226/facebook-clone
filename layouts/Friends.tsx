...
const Friends = () => {
  ...
  return (
    <div className="w-full mt-14 min-h-screen bg-[#18191a]">

      <aside
        aria-label="Friends navigation"
        className="hidden md:flex flex-col fixed left-0 top-14 rounded-xl w-90 h-[calc(100vh-3.5rem)] bg-[#242526] border-r border-[#3a3b3c] overflow-y-auto scrollbar-hide px-3 py-4 gap-y-2 z-10"
      >
        <header className="flex items-center justify-between px-2 mb-1">
          <h1 className="font-bold text-gray-100 text-[1.375rem] leading-tight">Friends</h1>
          <button
            aria-label="Friend settings"
            className="w-9 h-9 bg-[#3a3b3c] hover:bg-[#4e4f50] rounded-full flex items-center justify-center text-gray-300 hover:text-white transition-all duration-150 cursor-pointer"
          >
            <Settings size={18} />
          </button>
        </header>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[#3a3b3c] hover:bg-[#4e4f50] rounded-full pl-9 pr-4 py-2 text-[0.875rem] text-gray-200 placeholder:text-gray-400 outline-none transition-colors duration-150"
          />
        </div>
              ...
              <li key={link.label}>
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 group ${
                    link.active ? "bg-[#1877f2]/10 text-[#4da3ff]" : "text-gray-300 hover:bg-[#3a3b3c] hover:text-white"
                  }`}
                >
                  <span className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
                    link.active ? "bg-[#1877f2]/20 text-[#4da3ff]" : "bg-[#3a3b3c] text-gray-300 group-hover:text-white"
                  }`}>
                    {link.icon}
                  </span>
      ...
      <main aria-label="Friends content" className="md:ml-90 min-h-screen bg-[#18191a] px-4 py-5">
        <div className="max-w-375 mx-auto flex flex-col gap-y-6">

          <section aria-label="Friend requests">
            <div className="flex items-center justify-between mb-3 px-1">
              <h2 className="font-bold text-gray-100 text-[1.0625rem] leading-tight">Friend Requests</h2>
              <button className="text-[0.9375rem] font-semibold text-[#4da3ff] hover:text-[#6ab4ff] hover:underline transition-colors duration-150 cursor-pointer">
                See all
              </button>
            </div>

            {!requestersLoading && requesters.length === 0 ? (
              <p className="text-gray-400 text-[0.875rem] px-1">No friend requests</p>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-7 gap-3">
                {requestersLoading
                  ? [1, 2, 3].map((i) => <PeopleCard key={i} variant="friendsSection" />)
                  : requesters.map((user) => {
                      const isProcessing =
          ...
          <section aria-label="People you may know">
            <div className="flex items-center justify-between mb-3 px-1">
              <h2 className="font-bold text-gray-100 text-[1.0625rem] leading-tight">People You May Know</h2>
              <button className="text-[0.9375rem] font-semibold text-[#4da3ff] hover:text-[#6ab4ff] hover:underline transition-colors duration-150 cursor-pointer">
                See all
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-7 gap-3">
              {suggestionsLoading
                ? [1, 2, 3, 4, 5, 6].map((i) => <PeopleCard key={i} variant="profile" />)
